...
public class Filter {
  ...
  public static ArrayList<String> functionListForTC = new ArrayList<String>(); // for Test Case
                                                                               // filter
  public static ArrayList<String> testMethodList = new ArrayList<String>(); // for Test Method
                                                                            // filter
  ...
  private ArrayList<Arch_Channel> pArchitectureData = new ArrayList<Arch_Channel>(); // parsed
                                                                                     // architecture
                                                                                     // data
  ...
  private void collectFilterInfoForFirstPage() {
    ...
    for (i = 0; i < pLogData.size(); i++) {
      ...
      if (tempLogData.fileName.equals("null")) {
        System.out
            .println("Filter : Log Err - The fileName is null. Don't add this log to File list.");
      } else {
        if (fileList.size() == 0) {
      ...
      if (tempLogData.testSuiteName.equals("null")) {
        System.out.println(
            "Filter : Log Err - The testSuiteName is null. Don't add this log to Test case list.");
      } else {
      ...
      if (testMethodList.size() == 0) {
        if (tempLogData.action.equals("call")
          ...
          if (tempLogData.functionName.equals("<init>")) { // constructor call
            ...
            System.out.println(
                "Filter : Log Err - The functionName is null. Don't add this log to Test Method list.");
          } else {
            testMethodList
                .add(new String(tempLogData.calledClass + "." + tempLogData.functionName));
          }
        }
    ...
    for (i = 0; i < pLogData.size(); i++) {
      ...
      if (tempLogData.testSuiteName.equals(testCaseName)) {
        if (functionListForTC.size() == 0) {
          ...
          if (!tempLogData.calledClass.equals("null")
              && tempLogData.functionName.equals("<init>")) {
            mfunctionName = splitText[splitText.length - 1];
          } else {
            mfunctionName = tempLogData.functionName;
          }
          ...
          if (functionListForTC.size() == j && tempLogData.action.equals("call")) {
            if (tempLogData.functionName.equals("<init>")) { // constructor call
              functionListForTC.add(new String(tempLogData.calledClass));
            } else if (tempLogData.functionName.equals("null") || mfunctionName.equals("null")) {
              System.out.println(
  ...
  protected boolean setArchitectureNode(int filterRule, String inputParam1, String inputParam2) {
    ...
    if (filterRule == INTER_COMPONENT_FILTER) {
      for (i = 0; i < pArchitectureData.size(); i++) {
        ...
        // Graph node
        if (tempArch.start.contains(inputParam1) && tempArch.end.contains(inputParam2)) { // package1
                                                                                          // ->
                                                                                          // package2
        ...
        if (tempArch.end.contains(inputParam1) && tempArch.start.contains(inputParam2)) { // package2
                                                                                          // ->
                                                                                          // package1
          for (j = 0; j < tempArch.event.size(); j++) {
            gNodeTemp = new GraphNode();
    ...
    for (i = 0; i < pLogData.size(); i++) {
      ...
      if (tempLogData.testSuiteName.equals(testCaseName)) {
        if (tempLogData.action.equals(tempLogDataBackup.action)
            ...
            && tempLogData.functionName.equals(tempLogDataBackup.functionName)) { // skip duplicated
                                                                                  // log
          continue;
        } else {
          String[] splitText = tempLogData.calledClass.split("[.]");
          ...
          if (tempLogData.action.equals("call")) {
          ...
          } else if (!tempLogData.action.equals("null")) {
            if (gTextualTemp != null && mGroupOpenClose == true) {
            ...
            System.out
                .println("Filter : Log Err - The action is null. Don't add this log to text tree.");
          }
        }
